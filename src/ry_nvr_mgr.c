...
// 超前引用
void NvrAdpOnline(int nvr);


volatile int video_adp_state = 0;                   // video 接口初始化状态
RY_NVR_RECORD *ry_nvr_records = NULL;               // nvr配置的HASH
RY_NVR_INTERFACE *ry_nvr_interfaces = NULL;         // nvr驱动（动态链接库）、接口的对象
 ...
 * @return
 */
void NvrPtzCtl(int cmd, int nvr, int node, int parm) {
    ...
    for (nvr_interface = ry_nvr_interfaces; nvr_interface != NULL; nvr_interface = nvr_interface->hh.next) {
        ...
        if (nvr_interface->dll == NULL) {
            ...
            if (hDll != NULL) {
                ...
                nvr_interface->nvr_login = (NVR_ADP_LOG_IN) dlsym(hDll, "NvrAdpLogin");
                nvr_interface->nvr_ptz = (NVR_ADP_PTZ) dlsym(hDll, "NvrAdpPtz");
                nvr_interface->nvr_callback_reg = (NVR_ADP_CALLBACK_REG) dlsym(hDll, "NvrAdpCallbackReg");
                nvr_interface->nvr_free = (NVR_ADP_FREE) dlsym(hDll, "NvrAdpFree");
 ...
 * @return
 */
int NvrOnMsg(int nvr, int channel, int event, void *data) {
...
void NvrParseCtlPtz(char *msg) {
    ...
    NvrPtzCtl(PTZ_GOTO_PRESET, nvr, channel, ptz);
    /* cleanup code */
    json_value_free(root_value);
}
 ...
 * 消息格式: <cmd>+payload
 */
void NvrOnMqttMsg(int cmd, char *msg, int len) {
